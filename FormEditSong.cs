...
using Sunny.UI;
using System.Diagnostics; // Thêm using này để mở folder
    ...
    public partial class FormEditSong : UIForm
    {
        ...
        public FormEditSong(int? songId = null)
        {
            ...
            else
            {
                ...
                // Khi tạo mới, chưa có file đích -> vô hiệu hóa nút
                ...
                try
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            // Kích hoạt/Vô hiệu hóa nút dựa trên sự tồn tại của file
        ...
        // Mở thư mục chứa file nhạc của ứng dụng
        ...
        // Mở thư mục chứa ảnh bìa của ứng dụng
        ...
        private void btnSelectFile_Click(object sender, EventArgs e)
        {
            using (var ofd = new OpenFileDialog { Filter = "Audio Files (*.mp3;*.mp4;*.wav)|*.mp3;*.mp4;*.wav" })
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    txtFilePath.Tag = ofd.FileName; // Lưu đường dẫn gốc để sao chép

                    // Khi chọn file mới, nút mở thư mục đích chưa có ý nghĩa
                    // vì file chưa được sao chép. Ta có thể vô hiệu hóa nó.
                    btnOpenMusicFolder.Enabled = false;

                    // Tự động đọc thông tin từ file nhạc
                    try
                    {
        ...
        private void btnSelectCover_Click(object sender, EventArgs e)
        {
            using (var ofd = new OpenFileDialog { Filter = "Image Files (*.jpg;*.jpeg;*.png;*.bmp)|*.jpg;*.jpeg;*.png;*.bmp" })
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    btnOpenCoverFolder.Enabled = false; // Tương tự, file ảnh chưa được sao chép
                }
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            // Chỉ sao chép khi có file mới được chọn
